...
<p align="center">
  <img src="docs/hardware.jpg" width="55%" alt="실물 디오라마 - 침실 축소 모형에 LED 조명, 커튼, 선풍기, LCD를 배치"/>
</p>
...
<p align="center">
  <img src="docs/demo_curtain.gif" width="55%" alt="커튼 자동 개폐 시연"/>
</p>
...
| 분류 | 내용 |
|---|---|
| **MCU 제어** | ATmega128A 베어메탈 — 레지스터 직접 제어, 라이브러리/HAL 미사용 |
| **Timer/PWM** | Timer0/1/3 Fast PWM 직접 설정 — RGB LED 밝기 제어, 서보 2개(50Hz) 동시 구동, Timer1 3채널 자원 공유 |
| **ADC** | 조도 센서 폴링 방식 16회 평균 필터링 (AVCC 기준, 분주비 128) |
| **통신** | I2C(TWI) 마스터 직접 구현 — PCF8574 LCD 백팩 4비트 제어 / DHT11 단선 통신 타이밍 직접 구현(비트뱅잉) — 비트당 40us 판별, 인터럽트 차단으로 타이밍 보장 |
| **제어 기법** | 히스테리시스(채터링 방지), 서보 선형 가감속(관성 제거), 버튼 엣지 검출 + 디바운싱 |
| **소프트웨어 구조** | 논블로킹 슈퍼루프(협조적 스케줄링), 3계층 아키텍처(drivers/devices/app), Makefile 빌드 자동화 |
| **디버깅 경험 (팀)** | 돌입 전류·전압 강하 원인 분석 및 통신 방식 변경, 팀원 간 교차 검증으로 하드웨어 결함 격리, 인터럽트 크리티컬 섹션(cli/sei) 적용 |
...
| 팀원 | 담당 |
|---|---|
| **이현지 (본인)** | 선풍기 코드 구현, 환경 데이터 분석 및 알고리즘 설계 |
| 팀원 A | LED 코드 구현, 회로 구성 및 하드웨어 시스템 설계 |
| 팀원 B | 커튼 코드 구현, 회로 구성 및 하드웨어 시스템 설계 |
| 팀원 C | LCD 코드 구현 및 각 로직 통합, UART(블루투스) 어플리케이션 설계 |

**사후 보완 (이현지, 단독 수행)**: 프로젝트 종료 후 코드 품질 개선 작업
